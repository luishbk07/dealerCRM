...
export const LeadsInboxPage = () => {
  ...
  const activeLeadId = routeId ?? leads[0]?.id
  ...
  if (listQuery.isLoading) {
    return (
      <Box>
        <PageHeader title='Leads' subtitle='Cargando bandeja…' />
  ...
  if (listQuery.isError) {
    return (
      <Box>
        ...
        <ErrorAlert error={listQuery.error} onRetry={() => void listQuery.refetch()} />
      </Box>
    )
  }
  ...
  return (
    <Box>
      ...
      {leads.length === 0 ? (
        <EmptyState title='Todavía no tienes leads.' description='Los nuevos prospectos aparecerán aquí.' />
      ) : (
